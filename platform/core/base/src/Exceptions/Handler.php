        ...
        switch (true) {
            case $e instanceof TokenMismatchException:
                ...
                return $this->baseHttpResponse
                    ->setError()
                    ->setCode(419)
                    ->setMessage('Сессия администратора истекла. Войдите снова.')
                    ->setAdditional([
                        'login_url' => Route::has('access.login') ? route('access.login') : url('login'),
                    ])
                    ->toResponse($request);
                }

            if (is_in_admin(true) && Route::has('access.login')) {
                return redirect()
                    ->guest(route('access.login'))
                    ->with('error_msg', 'Сессия администратора истекла. Войдите снова.');
            }

            return redirect()
                ->back()
                ->withInput($request->except($this->dontFlash))
                ->with('error_msg', 'Сессия истекла. Повторите действие.');
