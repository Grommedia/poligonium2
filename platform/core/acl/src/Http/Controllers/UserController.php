...
class UserController extends BaseSystemController
{
    ...
    public function updatePreferences(User $user, PreferenceRequest $request)
    {
        PreferenceForm::createFromModel($user)
            ->saving(function (PreferenceForm $form) use ($request): void {
                 ...
                 */
                $model = $form->getModel();
                $locale = $this->normalizeAdminLocale($request->input('locale')) ?: $this->fallbackAdminLocale();

                $model->setMeta('locale', $locale);
                ...
                session()->put('admin_locale_direction', $request->input('locale_direction'));
    ...
    public function patchUpdatePreferences(User $user, PreferencePatchRequest $request)
    {
        ...
        if (array_key_exists('locale', $data)) {
            $data['locale'] = $this->normalizeAdminLocale($data['locale']) ?: $this->fallbackAdminLocale();
            session()->put('site-locale', $data['locale']);
            app()->setLocale($data['locale']);
        }

        if (array_key_exists('locale_direction', $data)) {
            session()->put('admin_locale_direction', $data['locale_direction']);
        }
