...
app()->booted(function () {
    Event::listen(RouteMatched::class, function (): void {
        ...
        Shortcode::register(
            'vfx-showreel',
            __('VFX Showreel'),
            __('Video-first portfolio section for VFX, product animation, commercials, and motion projects.'),
            function (ShortcodeCompiler $shortcode) {
                $items = [];

                if (class_exists(\Botble\VfxShowreel\Models\VfxShowreelItem::class)) {
                    $items = \Botble\VfxShowreel\Models\VfxShowreelItem::query()
                        ->wherePublished()
                        ->orderBy('order')
                        ->latest('created_at')
                        ->get()
                ...
                return Theme::partial('shortcodes.vfx-showreel.index', compact('shortcode', 'items'));
            }
        );

        Shortcode::setAdminConfig('vfx-showreel', function (array $attributes) {
            return ShortcodeForm::createFromArray($attributes)
                ->add(
                    'subtitle',
                    TextField::class,
                ...
                ->add(
                    'description',
                    TextareaField::class,
                    TextareaFieldOption::make()->label(__('Description'))->rows(3)
                )
                ->add(
                    'cta_text',
                    TextField::class,
                    TextFieldOption::make()->label(__('CTA text'))
                )
                ->add(
                    'cta_link',
                    TextField::class,
                    TextFieldOption::make()->label(__('CTA link'))
                )
                ->add(
                    'items',
                    ShortcodeTabsField::class,
                    ShortcodeTabsFieldOption::make()
                        ->label(__('Showreel cards'))
                        ->max(12)
                        ->fields([
                            'name' => [
                                'type' => 'text',
                            ...
                            'type' => [
                                'type' => 'text',
                                'title' => __('Type'),
                                'placeholder' => 'Product animation / VFX / Motion',
                            ],
                            'description' => [
                                'type' => 'textarea',
                                'title' => __('Description'),
                            ...
                            'preview_video' => [
                                'type' => 'text',
                                'title' => __('Preview video path or URL'),
                                'placeholder' => 'rabbit/banny-animation.mp4',
                            ],
                            'tools' => [
                                'type' => 'text',
                                'title' => __('Tools'),
                                'placeholder' => 'Blender, Houdini, After Effects',
                            ],
                            'year' => [
                                'type' => 'text',
                                'title' => __('Year'),
