...
@php
    ...
    $skillImage = function (array $tab): string {
        $image = trim((string) Arr::get($tab, 'image'));
        $name = BaseHelper::clean((string) Arr::get($tab, 'name'));

        if ($image && file_exists(public_path('storage/' . ltrim($image, '/')))) {
            return Html::image(url('storage/' . ltrim($image, '/')), $name, [
                'loading' => 'lazy',
                'data-bb-lazy' => 'true',
            ])->toHtml();
        }

        return RvMedia::image($image, $name)->toHtml();
    };
@endphp
                                    ...
                                    <div class="row">
                                        @foreach(collect($tabs)->split(2) as $row)
                                            <div @class(['mx-auto overflow-hidden', 'col-lg-10 col-md-9' => $loop->first, 'col-lg-8 col-md-7 col-10' => ! $loop->first])>
                                                <div @class(['carouselTicker position-relative z-1', 'carouselTicker-right' => $loop->first, 'carouselTicker-left' => ! $loop->first])>
