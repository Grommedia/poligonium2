    ...
    public function toResponse($request): JsonResponse|RedirectResponse
    {
        if ($request->expectsJson()) {
            ...
            if ($this->error && $this->code === 200) {
                if (preg_match('/csrf token mismatch|session expired/i', $message)) {
                    $this->code = 419;
                } elseif (preg_match('/unauthenticated/i', $message)) {
                    $this->code = 401;
                }
            }
            ...
            if (
                $this->error
                && in_array($this->code, [401, 419], true)
                && is_in_admin(true)
                && ! isset($this->additional['login_url'])
                && Route::has('access.login')
            ) {
                $this->additional['login_url'] = route('access.login');
            }
