@php
    ...
    $schoolAccountLabel = $isEnglish ? 'Sign in' : 'Вхід';
@endphp

<header>
    @if($sidebarPanelSidebar)
        <a href="#poligonium-contact-panel" title="{{ __('Get in touch') }}" class="poligonium-contact-tab" data-bs-target=".offCanvas__info" aria-label="{{ __('Get in touch') }}">
            <span class="poligonium-contact-tab__bolt" aria-hidden="true">
                <svg viewBox="0 0 24 24" focusable="false">
                    <path d="M13.6 2.4 5 13.1h6.2l-.9 8.5 8.7-11.1h-6.4l1-8.1Z" />
                </svg>
            </span>
            <span class="poligonium-contact-tab__label">{{ $isEnglish ? 'Contacts' : 'Контакти' }}</span>
        </a>
        <span id="poligonium-close-panel" class="poligonium-close-anchor" aria-hidden="true"></span>
    @endif
    <nav class="navbar navbar-expand-lg navbar-light w-100 flex-nowrap z-999 p-0">
        <div class="container py-3 px-0">
            <a class="navbar-brand d-flex main-logo align-items-center ms-lg-0 ms-md-5 ms-3" href="{{ BaseHelper::getHomepageUrl() }}">
                ...
                <span class="fs-4 ms-2 site-name-text poligonium-brand-wordmark">{{ $siteName }}</span>
            </a>
            ...
            <div class="navbar-social d-flex align-items-center pe-5 pe-lg-0 me-5 me-lg-0 gap-3">
                <a href="{{ auth()->check() ? route('courses.student.cabinet') : route('courses.student.login') }}" class="poligonium-school-account-link d-none d-lg-inline-flex">
                    {{ $schoolAccountLabel }}
                </a>
    ...
    @if(! empty($sidebarPanelSidebar))
        <div class="offCanvas__info" id="poligonium-contact-panel">
            <div class="offCanvas__close-icon menu-close">
                <a href="#poligonium-close-panel" title="{{ __('Close') }}"><i class="ri-close-line"></i></a>
            </div>
    ...
    @endif
    <a href="#poligonium-close-panel" class="offCanvas__overly" aria-label="{{ __('Close') }}"></a>
        ...
        <div class="mobile-header-wrapper-inner">
            <div class="mobile-header-logo">
                <a class="d-flex main-logo align-items-center d-inline-flex" href="{{ BaseHelper::getHomepageUrl() }}">
                    ...
                    <span class="fs-4 ms-2 site-name-text poligonium-brand-wordmark text-dark">{{ $siteName }}</span>
                </a>
                <div class="burger-icon burger-icon-white border rounded-3">
                    <span class="burger-icon-top"></span>
                    <span class="burger-icon-mid"></span>
            ...
            <div class="mobile-header-content-area">
                <div class="perfect-scroll">
                    ...
                    <a href="{{ auth()->check() ? route('courses.student.cabinet') : route('courses.student.login') }}" class="poligonium-school-account-link is-mobile">
                        {{ $schoolAccountLabel }}
                    </a>
